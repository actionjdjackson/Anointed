        ...
        if missingKnowledgeFlag == true {
            ...
            for child in (parent!.parent?.children)! {  //for all children in the menu
                ...
                if child is StudySectionLabel {     //if it's a study section label
                    ...
                    let enlarge = SKAction.scaleBy(3, duration: 15) //make an enlargement action
                    let fadeout = SKAction.fadeOutWithDuration(1.5) //make a fade out action
                    let fadein = SKAction.fadeInWithDuration(2.5)   //make a fade in action (for the resulting understanding element at end of meditation cycle)
